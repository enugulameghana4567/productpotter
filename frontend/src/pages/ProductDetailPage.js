...
const DEFAULT_PRODUCTS = {
  ...
  'default4': {
    ...
    colorDescription: 'Warm beige/linen background with sage green and golden botanical accents.',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'start' }}>
        ...
        <div style={{ position: 'sticky', top: 100 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 14 }}>
            <div style={{ width: 28, height: 4, borderRadius: 2, background: '#1a56db' }} />
            <div style={{ width: 8, height: 4, borderRadius: 2, background: '#dbeafe' }} />
            <div style={{ width: 8, height: 4, borderRadius: 2, background: '#dbeafe' }} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 2, fontFamily: "'Lato',sans-serif" }}>
            Potters Productions
          </div>

          <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 30, color: '#0e3a8c', lineHeight: 1.3, margin: '0 0 10px 0' }}>
            {product.name}
          </h1>
          ...
          <p style={{ color: '#374151', fontSize: 15, lineHeight: 1.9, margin: '0 0 18px 0' }}>
            {product.description}
          </p>
            ...
            <div style={{ padding: '16px 18px', background: '#f8faff', borderRadius: 12, border: '1.5px solid #dbeafe', marginBottom: 24 }}>
              {product.colorDescription && (
                <p style={{ fontSize: 13, color: '#374151', margin: '0 0 8px 0', lineHeight: 1.6 }}>
                  <strong style={{ color: '#0e3a8c' }}>Color:</strong> {product.colorDescription}
                </p>
              )}
              {product.designDescription && (
                <p style={{ fontSize: 13, color: '#374151', margin: '0 0 8px 0', lineHeight: 1.6 }}>
                  <strong style={{ color: '#0e3a8c' }}>Design:</strong> {product.designDescription}
                </p>
              )}
              {product.themeDescription && (
                <p style={{ fontSize: 13, color: '#374151', margin: 0, lineHeight: 1.6 }}>
                  <strong style={{ color: '#0e3a8c' }}>Theme:</strong> {product.themeDescription}
                </p>
              )}
            </div>
          )}
          ...
          <div style={{ borderTop: '1.5px solid #dbeafe', paddingTop: 24, marginBottom: 24 }}>
            <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, color: '#0e3a8c', margin: '0 0 20px 0' }}>
              Materials
            </h3>
              ...
              {materials.map(m => {
                ...
                return (
                  <div key={m._id} onClick={() => setSelectedMaterial(m)}
                    style={{ cursor: 'pointer', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      width: 60, height: 60, borderRadius: '50%',
                      background: m.color,
                      border: isSelected ? '3px solid #1a56db' : `3px solid ${isLight ? '#d1d5db' : m.color}`,
                      boxShadow: isSelected ? '0 0 0 3px rgba(26,86,219,0.20)' : '0 2px 8px rgba(0,0,0,0.12)',
                      transition: 'all .2s',
                      display: 'flex', alignItems: 'center', justifyContent: 'center'
                    }}>
                      {isSelected && (
                        <span style={{ color: isLight ? '#1a56db' : '#fff', fontSize: 20, fontWeight: 900, lineHeight: 1 }}>✓</span>
                      )}
                    </div>
                    <span style={{ fontSize: 13, color: isSelected ? '#1a56db' : '#374151', fontWeight: isSelected ? 700 : 400, fontFamily: "'Lato',sans-serif" }}>
                      {m.name}
                    </span>
                  </div>
                );
            ...
            {selectedMaterial && (
              <p style={{ fontSize: 13, color: '#6b7280', fontStyle: 'italic', margin: 0, lineHeight: 1.6 }}>
          ...
          <button onClick={handleOrderBooking} disabled={booking}
            style={{
              width: '100%', background: booking ? '#93c5fd' : '#1a56db', color: '#fff',
              border: 'none', borderRadius: 14, padding: '17px', fontSize: 18,
              fontWeight: 700, cursor: booking ? 'not-allowed' : 'pointer',
